...
exec @stgPackageId = [cfg].AddSSISPackage 'Get_dbo_tblDashboardAmount.dtsx', '' ;-- Folder name not needed for SSIS Catalog
SET @stgExecutionOrder += 10;
exec [cfg].[AddSSISProjectPackage] @stgProjectId, @stgPackageId, @stgExecutionOrder, 'I', 'F', 1;
...
-- App setings for Done Project (3499)
SET @stgAppName = 'SDT stgToJNT (End of week)';
exec [cfg].[AddSSISApplication] @stgAppName, @stgAppId OUTPUT;
exec [cfg].[AddSSISApplicationProject] @stgAppId, @stgProjectId, 10;

SET @stgExecutionOrder = 10;
exec @stgPackageId = [cfg].AddSSISPackage 'Mark_Journey_Posted.dtsx', '' ;-- Folder name not needed for SSIS Catalog
exec [cfg].[AddSSISProjectPackage] @stgProjectId, @stgPackageId, @stgExecutionOrder, 'I', 'F', 1;
...
;WITH CTE AS	(
				...
				FROM	(
						...
						UNION
							SELECT 4, 'mobile', 'tblDashboardAmount (Payslips)', 0, GETDATE(), PackageId FROM cfg.Packages WHERE PackageName = 'Get_dbo_tblDashboardAmount.dtsx'
						UNION

/*
Deployment script for Sastuma_Config
...
GO
:setvar DatabaseName "Sastuma_Config"
:setvar DefaultFilePrefix "Sastuma_Config"
...
AS
	DECLARE @JNTChangeTrackingID INT;

	SELECT	@JNTChangeTrackingID = JNTChangeTrackingID
	FROM	cfg.JNTChangeTracking	c 
	JOIN	cfg.Packages			p ON c.PackageId = p.PackageID  
	WHERE	PackageName				= @PackageName;

	SELECT ISNULL(@JNTChangeTrackingID, -1) AS JNTChangeTrackingID;
GO
...
exec @PackageId = [cfg].AddSSISPackage 'weblogs_sdt_logattribute.dtsx', '' ;
SET @ExecutionOrder += 10;
exec [cfg].[AddSSISProjectPackage] @ProjectId_Weblogs, @PackageId, @ExecutionOrder, 'I', 'F', 1;

exec @PackageId = [cfg].AddSSISPackage 'weblogs_sdt_logAttributeMapping.dtsx', '' ;
SET @ExecutionOrder += 10;
exec [cfg].[AddSSISProjectPackage] @ProjectId_Weblogs, @PackageId, @ExecutionOrder, 'I', 'F', 1;
...
SET @ExecutionOrder += 10;
exec @PackageId = [cfg].AddSSISPackage 'Load CustomerStatusTable.dtsx', '' ;
exec [cfg].[AddSSISProjectPackage] @ProjectId, @PackageId, @ExecutionOrder, 'I', 'F', 1;
...
--Sync from Assessment should run last

exec @stgPackageId = [cfg].AddSSISPackage 'Merge_dbo_CustomerDetailsAssessment.dtsx', '' ;-- Folder name not needed for SSIS Catalog
SET @stgExecutionOrder += 10;
exec [cfg].[AddSSISProjectPackage] @stgProjectId, @stgPackageId, @stgExecutionOrder, 'I', 'F', 1;

exec @stgPackageId = [cfg].AddSSISPackage 'Merge_dbo_CustomerTableAssessment.dtsx', '' ;-- Folder name not needed for SSIS Catalog
SET @stgExecutionOrder += 10;
exec [cfg].[AddSSISProjectPackage] @stgProjectId, @stgPackageId, @stgExecutionOrder, 'I', 'F', 1;
...
;WITH CTE AS	(
				...
				FROM	(
						...
						UNION
							SELECT 8, 'dbo', 'CustomerTableAssessment', 0, GETDATE(), PackageId FROM cfg.Packages WHERE PackageName = 'Merge_dbo_CustomerTableAssessment.dtsx'
						UNION
							SELECT 9, 'dbo', 'CustomerDetailsAssessment', 0, GETDATE(), PackageId FROM cfg.Packages WHERE PackageName = 'Merge_dbo_CustomerDetailsAssessment.dtsx'
						) AS ctTable
...
declare @PackageId INT = 0;
declare @ExecutionOrder INT = 0;
declare @AppName VARCHAR(256) = 'SDT Hubload CRA';
declare @ProjectName VARCHAR(256) = 'SDT Hubload CRA';
...
SET @ExecutionOrder += 10;
exec @PackageId = [cfg].AddSSISPackage 'Load CustomerExpenditure.dtsx', '' ;
exec [cfg].[AddSSISProjectPackage] @ProjectId, @PackageId, @ExecutionOrder, 'I', 'F', 1;
GO
declare @MergeAppId INT = 0;
declare @MergeProjectId INT = 0;
declare @MergePackageId INT = 0;
declare @MergeExecutionOrder INT = 0;
declare @MergeAppName VARCHAR(256) = 'SDT HubMerge CRA';
declare @MergeProjectName VARCHAR(256) = 'SDT HubMerge CRA';
...
exec @MergePackageId = [cfg].AddSSISPackage 'Merge_dbo_tblCustomerExpenditure.dtsx', '' ;-- Folder name not needed for SSIS Catalog
exec [cfg].[AddSSISProjectPackage] @MergeProjectId, @MergePackageId, @MergeExecutionOrder, 'I', 'F', 1;
